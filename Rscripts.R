...
getChordDiagram <- function ( module, transporternum = 50, targetnum = 15, ratio = 1, cex = 1)
{
	if (!file.exists("results.regulator.transporters.txt")){
		cat ("The file 'results.regulator.transporters.txt' not found.\n")
		return("Exit")
	}

	if (!file.exists("getChordLists.pl")){
		cat ("The file 'getChordLists.pl' not found. This file is required for extracting the transporter-non-transporter target gene pairs for the input module. It should be placed within the home folder of the EXPLICIT-Kinase package.\n")
		return("Exit")
	}
	a <- read.table("results.regulator.transporters.txt",heade=T,sep="\t")
	if ( module %in% a[,1]){
		unlink( "chord.lists.txt" ) 
		perlcmd = paste("perl getChordLists.pl",module,transporternum,targetnum,sep=" ")
		system( perlcmd )
		...
		cat ("The module ",module," was not found in 'results.regulator.transporters.txt'.","\n",sep="")
	}
}
