use strict;
my $pvalue_cutoff = 1E-09;  			# pValue cut-off for transporter - non-transporter gene pair.  
my $qvalue_cutoff = 0.05;  			# qvalue cut-off for a transporter to be considered as predictor transporter for a module.
my $total_gene = 27402; 			# the total number of non-transporter genes.
...
my $target_gene_file = "./data/At.Transporters.Targets.txt";
my $gene_symbl_file = "./data/At.gene.symbls.txt";
#my $edge_file = "./data/At.SigEdges.txt"; 		# with singinificant interacting transporter-target gene pair.
#my $edge_file = "./data/At.SigEdges.txt.bk";
my $edge_file = "./data/At.Transporters.SigEdges.txt";
my $output_file = "results.regulator.transporters.txt";
...
my %e;		# to store predictor transporters info for target genes.
my %tfc;	# number of target genes for a transporter within the genome.
my %seen;
my $l;
my @t;
my %symbl;
...
unless( -e $edge_file ){
	print "The file 'At.SigEdges.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getArabidopsisRegulatorTransporters.pl' should not be changed.\nExit in 5 second.\n";
	sleep 5;
	exit 1;
}

unless( -e $gene_symbl_file ){
	print "The file 'At.gene.symbls.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getArabidopsisRegulatorTransporters.pl' should not be changed.\nExit in 5 seconds.\n";
	sleep 5;
	exit 1;
}

unless( -e $target_gene_file ){
	print "Target gene file 'At.target.genes.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getArabidopsisRegulatorTransporters.pl' should not be changed.\nExit in 5 seconds.\n";
	sleep 5;
	exit 1;
}

unless( -e $cluster_file ){
	print "The file 'modules_to_analyze.txt' not found. It should be saved in the same directory as the script 'getArabidopsisRegulatorTransporters.pl'.\nExit in 5 seconds.\n";
	sleep 5;
	exit 1;
}
...
print OUT "Module\tRank\tTransporter\tSymbl\tModuleSize(without transporter genes)\tCount\tCountInGenome\tGenomeSize\tpValueEnrichment\tpValue(bh-adjusted)\tFraction\tmean_beta\tTargetAGI\tTargetSymbl\tbeta\tTargetpValue( -log10 p )\n";
foreach $k ( @c ){
	$l = $g{$k};
	$l =~ s/\t$//;
	@t = split("\t",$l);
